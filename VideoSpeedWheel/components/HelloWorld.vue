...
// 1. 导入 WXT storage API 和站点配置匹配函数（保持不变）
...
// 4. 解析站点名称（用于UI显示）
const getSiteName = (storageKey) => {
  const siteMap = {
    'local:bilibili_video_speed_config': '哔哩哔哩',
    'local:xiaohongshu_video_speed_config': '小红书',
    'local:douyin_video_speed_config': '抖音',
    'local:kuaishou_video_speed_config': '快手',
    'local:youtube_video_speed_config': 'YouTube',
  };
  return siteMap[storageKey] || '未知站点';
};
...
const loadConfig = async () => {
  try {
    ...
    // 初始化动态存储项
    currentSite.value = getSiteName(matchedConfig.storageKey);
    speedConfigItem.value = storage.defineItem(matchedConfig.storageKey, {
      init: () => ({ ...DEFAULT_CONFIG, ...matchedConfig.defaultConfig }), // 合并站点默认配置
    });
